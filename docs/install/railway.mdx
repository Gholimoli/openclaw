...
## When Railway is not a fit

Railway is a great zero-ops starter, but it is not the best match when you want:

- a Gateway that is not publicly reachable (loopback-only with private access)
- a long-running, CLI-heavy coding automation pipeline that depends on local binaries and local credential stores
- strict operator-only controls (Tailscale access, host-level hardening, custom sandboxes)

For those cases, prefer a private VPS Gateway:

- Secure setup guide: [VPS coding automation](/install/vps-coding)
- Pipeline design: [Coding automation pipeline](/automation/coding-pipeline)
...
## Model defaults (OpenRouter primary + fallback)

If you prefer to set your model defaults directly in config, use:
...
- OpenRouter model refs are `openrouter/<provider>/<model>`. See [OpenRouter](/providers/openrouter).
- Fallbacks are tried when all auth profiles for the current provider fail. See [Model failover](/concepts/model-failover).
